...
//    LOGD("bird: input calc length: %d ", myLength);
    ...
    char *src = new char[myLength];
    memset(src, 0, myLength);
    strcpy(src, encodeContent);

    AES_CBC_encrypt_buffer((uint8_t *) buffer, (uint8_t *) src, myLength, key, iv);

    jbyteArray jarray = jniEnv->NewByteArray(myLength);
    jniEnv->SetByteArrayRegion(jarray, 0, myLength, (const jbyte *) buffer);

    jniEnv->ReleaseStringUTFChars(content, encodeContent);
...
//    LOGD("bird: decode calc length: %d ", strlen(buffer2));
    ...
    jniEnv->ReleaseByteArrayElements(content, (jbyte *) olddata, JNI_COMMIT);
